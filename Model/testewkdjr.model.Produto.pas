...
begin
  ...
  try
    ...
    cdsDados.Data := objDA.ExecutaDataset('SELECT * FROM Produto WHERE ProdutoId = '
                                  + IntToStr(iProdutoId));
...
begin
  objDA := TDataAcess.Create;
  try
    Result := objDA.ExecutaDataset('SELECT * FROM Produto ORDER BY Descricao');
  finally
    objDA.Free;
  end;
end;
